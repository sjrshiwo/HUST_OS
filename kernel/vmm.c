...
void *user_va_to_pa(pagetable_t page_dir, void *va) {
  ...
  // invalid PTE, and should return NULL.
   pte_t pa=lookup_pa(page_dir,(uint64)va);
  //  panic("%lld\n",pa);
  //  pte_t* pte = page_walk(page_dir,(uint64)va,0);
   //panic("%lld\n",*pte);
  return (void *)(pa +((uint64)va&0xfff)) ;
...
void user_vm_unmap(pagetable_t page_dir, uint64 va, uint64 size, int free) {
  ...
  if(free!=0)
  {
    pte_t * pa=page_walk(page_dir,va,0);
    //pte_t pa=lookup_pa(page_dir,  va);
    ...
    *pa &=(~PTE_V);
    ...
    //     pte_t pa=lookup_pa(page_dir,  va+i);//物理地址删除，vaild标识为0
        
    //     pa&0xfffffffffffffffe;
    // }
